...
const Ticket = ({ price, carrier, segments }) => {
  ...
  const destinationTimeTo =
    new Date(Date.parse(segments[0].date) + segments[0].duration * 60000).toTimeString().slice(0, 5);
  const originTimeBack = new Date(segments[1].date).toTimeString().slice(0, 5);
  const destinationTimeBack =
    new Date(Date.parse(segments[0].date) + segments[1].duration * 60000).toTimeString().slice(0, 5);
    ...
    <div className={classes.Ticket}>
      ...
      <div className={classes.Ticket__direction}>
        <div className={classes.Ticket__part}>
          ...
          <div className={classes.Ticket__info}>{originTimeTo} - {destinationTimeTo}</div>
        </div>
        <div className={classes.Ticket__part}>
          <div className={classes.title}>В ПУТИ</div>
          <div className={classes.Ticket__info}>{Math.floor(segments[0].duration / 60)}ч{segments[0].duration%60}м</div>
        </div>
        <div className={classes.Ticket__part}>
      ...
      <div className={classes.Ticket__direction}>
        <div className={classes.Ticket__part}>
          ...
          <div className={classes.Ticket__info}>{originTimeBack} - {destinationTimeBack }</div>
        </div>
        <div className={classes.Ticket__part}>
          <div className={classes.title}>В ПУТИ</div>
          <div className={classes.Ticket__info}>{Math.floor(segments[1].duration / 60)}ч{segments[1].duration%60}м</div>
        </div>
        <div className={classes.Ticket__part}>
